...
class Channel:
    ...
    async def recent(self, t, payload):
        ...
        # async with await open_file(file_name, 'rb') as inf:
        #     await inf.seek(0, 2)
        #     tell = await inf.tell()
    ...
    async def stats(self, t, payload):
        """
    ...
    def _get_index(self):
        return self._indexes.get(self.channel_name)

    def set_cache(self, key, value):
        key = f'{self.channel_name}:{key}'
        self._cache[key] = value
        self._indexes[self.channel_name].append(key)
